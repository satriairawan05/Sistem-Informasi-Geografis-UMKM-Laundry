...
<div class="container-fluid">
  <form class="form-horizontal" method="post" action="/dashboard/status/{{ $statuses->id }}">
    @method('put')
    @csrf
      ...
      <div class="form-group row">
        ...
        <div class="col-md-6">
          <input type="text" class="form-control @error('status')
                  is-invalid
              @enderror" id="status" name="status" placeholder="Status" required value="{{ old('status',$statuses->status) }}" />
        @error('status')
